...
\item This has a hidden upward selection bias, or ``winner's curse'', as we are only testing this hypothesis as Trump is leading in the polls. In other words, the hypothesis itself is random. Alternatively, we can note that the marginal type I error rate is indeed not so straightforward. If the null hypothesis is ``$H_0: \text{Leader in polls has most support}$'' \WFcomment{Careful, this is an event}, then for any distribution $F$ the marginal type I error rate is
...
\item Comparing Trump to all other candidates involves multiple comparisons. Without considering the structure of this problem, common methods like Tukey's correction \cite{Tukey:1951} may largely decrease the test power \WFcomment{Tukey's correction doesn't directly cover this case unless we modify it somehow. But without discussing how we might carry out this modification, bringing up Tukey might confuse people.}
...
when $X_j$ is larger than all other $X_k$. \WFcomment{This a long and overly-abstract way of saying ``we break ties randomly''} To break ties, we introduce independent auxiliary random variable $U_j \sim U\left[0,1\right]$, and opt to test $H_{0j}$ in the event of
...
\item multiply the first term in the denominator by $1/2$ if $X_1$ and $X_2$ has the same parity \WFcomment{could be a bit clearer about why you do this}.
\end{itemize}
Noticing that if there are ties for the largest the $p$-value is $1$ and the denominator is really $1/2$ otherwise \WFcomment{I don't think that's true for $j \neq 2$, is it?}, we can write
...
meaning that it is sufficient to construct a one-sided test between $X_1$ and $X_2$ and consider only $p_{12}$. \WFcomment{This derivation is a little unclear. Is the middle probability a well-defined probability?}

It may appear quite surprising that ``verifying a winner'' as we propose in this section requires no adjustment for multiplicity, in the sense that we only need to compare the winner to the runner- up, just as we would have done if $n = 2$ \WFcomment{I don't think it's clear enough to readers why the test is ``just as we would have done if $n=2$''}. After all, we know that the winner suffer’s from a winner's curse: given $X_5$ is the winner, $X_5 / m$ would overestimate $\pi_5$, and the larger $n$ is, the more severe this bias is likely to be. However, if $X_9$ is the runner-up, then $X_9 / m$ is likely biased upward nearly as much as $X_5$. Informally, we might say that $X_9$ suffer’s from a “runner-up’s curse” that is nearly as bad as the winner's curse; as a result, $X_5 - X_9$ is actually not all that biased upward. By multiplying the $p$-value by $2$, or equivalently performing a two-tailed test, we take care of what little bias there is. Indeed, performing a two-tailed test can be viewed as a sort of ``correction for multiplicity'', albeit one that we would have to make even if $n = 2$.
...
for various combinations of $m$ and $n$. For Gupta and Nagel, we use $\alpha = 0.05$; but to account for the extra factor of $\frac{n-1}{n}$ in \Cref{eqn:selective_vs_marginal}, we will apply the selective procedure for with $\frac{n}{n-1} \alpha$ such that the marginal type I error rate such that both procedures are controlled at $\alpha$. \WFcomment{Can we justify this? Our procedure has error $\alpha$ in the case where all $\pi_j$ are equal.} Gupta and Nagel coincides with our test at $n = 2$; however as $n$ grows, the selective test shoes significantly more power than Gupta and Nagel's test.
...
\begin{tikzpicture}
	...
	\draw[dashed] (0, 0) -- (C1);
	\draw[dashed] (0, 0) -- (C2);
	\fill[yellow!90!gray] (0, 0) -- (C1) -- ($(C1) + (7, 0)$) -- ($(C2) + (8, 0)$) -- (C2) -- cycle;
...
\begin{tikzpicture}
	...
	\fill[yellow!90!gray] (0, 0) -- (C1) -- (C5) -- (C6) -- (C2) -- cycle;
