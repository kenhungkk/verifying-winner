...
\begin{abstract}
Many statistical experiments involve comparing of multiple population groups. For example, a public opinion poll may ask which of several political candidates commands the most support in a given population, or a clinical trial may compare patient outcomes under several treatment conditions. This article concerns the problem of {\em rank verification} --- post hoc tests of whether the ordering discovered in the data is in fact statistically significant. For exponential family models, we show under mild conditions that an unadjusted two-tailed pairwise $p$-value comparing the ``winning'' population to the ``runner-up'' is a valid $p$-value for checking whether the winner is truly the best. We extend this result to provide lower confidence bounds on the gap between the ``winning'' population and the others, and propose a simple step-down algorithm for verifying ranks beyond the first.
\end{abstract}
...
On February 1, 2016, Quinnipiac University released a poll of 890 Iowa Republicans regarding their preferred candidate for the Republican presidential nomination \citep{quinnipiac}. The results, shown in Table~\ref{tbl:poll}, placed Donald Trump in the lead with $31\%$ of the vote, with Ted Cruz in second with $24\%$, Marco Rubio in third with $17\%$, and ten other candidates including ``Don't know'' trailing behind. We assume for simplicity that the poll represents a simple random sample of Iowa Republicans, i.e., that the data are a multinomial sample of size $890$ and underlying probabilities $(\pi_{\text{Trump}}, \pi_{\text{Cruz}}, \pi_{\text{Rubio}}, \ldots)$.\footnote{The reality is a bit more complicated: before releasing the data, Quinnipiac has processed it to make the reported result more representative of likely caucus-goers.}
...
Seeing that Trump leads this poll, several salient questions may occur to us: Is Trump really winning, and if so by how much? And analogously, is Cruz really in second, is Rubio really in third, and so on?
...
Nevertheless, if we blithely ignore the selection issue, we might carry out the following analyses to answer the questions we posed before at significance level $\alpha=0.05$:
\begin{enumerate}
...
\item {\em Is Cruz in second, Rubio in third, etc.?} We can next compare Cruz to Rubio just as we compared Trump to Cruz (again rejecting because 214 is significantly more than half of 365), then Rubio to Carson, and so on, as a stepdown procedure. The first four comparisons are all significant at level $0.05$, but Paul and Bush are tied so we stop.
\end{enumerate}

Surprisingly, the three procedures described above are all entirely valid despite the implicit multiple-comparisons issue. The remainder of this article is dedicated to justifying them for the multinomial family and many other settings.
...
We now state our main theorem: under a mild technical assumption, the three procedures described above are statistically valid, even accounting for the selection.
\begin{theorem}\label{thm:main}
Assume the model~\eqref{eq:expfam} holds and $g(x)$ is a Schur-concave function. The first procedure has level $\alpha$ conditional on the best population not winning (otherwise an error is impossible). The second procedure gives an exact $1-\alpha$ lower confidence bound for $\theta_{[1]} - \max_{j \neq [1]} \theta_{j}$. The third procedure is a conservative stepdown procedure with familywise error rate (FWER) no larger than $\alpha$.
\end{theorem}

We define Schur-concave functions and discuss their properties in Section~\ref{sec:maj}. Because any log-concave and symmetric function is Schur-concave, Theorem~\ref{thm:main} applies to all of the cases discussed above.

Note that there is an implicit adjustment for selection here, since we are using two-tailed unadjusted tests. If we instead based our tests on an independent realization $X^*$, then we could (for example) use the right-tailed version of the unadjusted pairwise test for the first procedure. Having to use a two-tailed procedure is in some sense unsurprising: Even for $n=2$, it is intuitive that we should have to use the two-tailed test instead of the one-tailed. Indeed, the first procedure is exactly equivalent to the two-tailed test of the fixed hypothesis $\theta_1=\theta_2$. 

What may be more surprising is that, no matter how large $n$ is, we do not require any additional adjustment beyond what we already require for $n=2$. Informally, this can possibly be explained by noting that for large $n$, $X_{[2]}$ suffers from a positive selection bias --- a ``runner-up's curse,'' so to speak --- which is nearly as severe as the bias of $X_{[1]}$.
