...
\begin{proof}
The type I error --- the probability of incorrectly declaring anyone other than the best to be the best --- is
\[
\PP\left[\bigcup_{j \text{ not best}} \text{declare } j \text{ best}\right] \leq \sum_j \PP\left[ \text{declare } j \text{ best} \mid j \text{ wins}\right] \PP[j \text{ wins}].
\]
Because only one candidate can win (recall that ties are broken randomly), it is sufficient to show for each $j$ that 
$\PP_{H_{0j}}\left[ \text{reject } H_{0j} \mid j \text{ wins}\right] \leq \alpha$. 
...
\WFcomment{It also relies on the atoms on the two rays ``matching up,'' right?}
\end{proof}
...
$$p = 2\PP\left[\text{Binomial}\left(8 + 5, 0.5\right) \ge 8\right] = 0.58,$$
