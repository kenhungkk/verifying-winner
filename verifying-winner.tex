...
A Schur-concave function is symmetric by default \WFcomment{``since $a \succeq b$ and $b \succeq a$ if $b$ is a permutation of the coordinates of $a$''}. Conversely a symmetric and log-concave function is Schur-concave \WFcomment{Is this obvious?}, thus many common exponential family distributions lie in this class. Schur-concave function indeed appears a lot, \Cref{sec:applications} includes both continuous and discrete examples.

Now suppose the carrier distribution $g$ in \Cref{eqn:exp_family} is indeed Schur-concave. We hereon call the class of distribution satisfying this {\em Schur-concave exponential family}. Without loss of generality, we can assume $X_1 \ge X_2 \ge \max_{k > 2} X_k$ \WFcomment{Careful here: I know what you mean but some people will find it confusing to ``assume'' that an event occurs. Maybe say ``without loss of generality, we consider the case where...; other cases follow by relabeling.}. We will be testing the null hypothesis $H_{01}: \theta_1 \le \max_{j > 1} \theta_j$, which is the union of the null hypotheses $H_{0j}: \theta_1 \le \theta_j$ for $j > 1$ \WFcomment{Notation could be confusing: $H_{01}$ is not a special case of $H_{0j}$}.
...
Integrating both sides over the region $\left\{X: X_1 \ge a, X_2 < a\right\}$, summing instead if $X$ is a discrete vector \WFcomment{it's not necessary to distinguish summing from integrating since summing is ``integrating wrt counting measure.'' Anyway the carrier measure could be neither discrete nor absolutely continuous}, gives
...
with a Schur-concave carrier measure $g$. Then there is a selective level-$\alpha$ test that verifies $\theta_{\left(1\right)} > \theta_{\left(2\right)}$ \WFcomment{I know what you mean here but notation could be confusing} by comparing only $X_{\left(1\right)}$ and $X_{\left(2\right)}$. The test also verifies that $X_{\left(1\right)} \ge X_{\left(j\right)}$ stochastically for all $j > 1$. Here $X_{\left(j\right)}$ is the order statistic where $X_{\left(1\right)} \ge \cdots \ge X_{\left(n\right)}$.
\label{thm:main_result}
\end{theorem}

\WFcomment{This result could be stated a bit more strongly, though I'm not certain exactly how to phrase it. In fact the test we perform is whatever is the ``usual'' two-sided level-$\alpha$ test for comparing $\theta_1$ vs. $\theta_2$.

The proof and figures look great!}
...
\begin{tikzpicture}
	...
	\draw[dashed] (0, 0) -- (C1);
	\draw[dashed] (0, 0) -- (C2);
	\fill[yellow!50] (0, 0) -- (C1) -- ($(C1) + (7, 0)$) -- ($(C2) + (8, 0)$) -- (C2) -- cycle;
...
\begin{tikzpicture}
	...
	\fill[yellow!50] (0, 0) -- (C1) -- (C5) -- (C6) -- (C2) -- cycle;
