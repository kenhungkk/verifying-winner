...
\Cref{fig:compare_rays} have both the $p$-values shown on the same diagram. Proving $p_{12} \ge p_{13}$ is the same as proving
$$\frac{B}{A+B} \ge \frac{D}{C+D} ~~~~ \Longleftrightarrow ~~~~ \frac{B}{A} \ge \frac{D}{C}.$$
We will prove so be extending $A$ to include $A^*$ on the diagram. We denote the sum as $A'$. Formally,
\begin{equation}
...
\begin{tikzpicture}
	...
	\draw[thick, dashed] (A) -- (D) node[midway, below left] {$A^*$};
