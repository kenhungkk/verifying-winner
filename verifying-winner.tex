...
Here, we consider only the case where the winner is $1$ and the runner-up is $2$, etc., i.e.\ $X_1 \ge \cdots \ge X_n$ and that ties are broken uniformly. For other cases, the following proof remains valid under relabeling. In this case, we will test the null hypothesis $H_{01}: \theta_1 \le \max_{j > 1} \theta_j$, which is the union of the null hypotheses $H_{01j}: \theta_1 \le \theta_j$ for $j > 1$. For each of these we can construct an exact $p$-value $p_{1j}$. Hence by \Cref{lma:union}, a test based on $p_{1*} = \max_j p_{1j}$ would be valid for $H_{01}$. Procedure 1 performs an unadjusted pairwise test comparing $X_1$ to $X_2$. Hence it is sufficient to show that $p_{12} = p_{1*}$ and that the test based on $p_{12}$ coincides with the unadjusted pairwise test.

Our proof has three main parts: (1) deriving $p_{1j}$ for each $j\geq 2$, (2) showing that $p_{12} \geq p_{1j}$ for each $j\geq 2$, and 
(3) showing that $p_{12}$ is an unadjusted pairwise $p$-value.


\paragraph{Derivation of $p_{1j}$}
Following the framework in \citet{Fithian:2014ws}, we consider the selection event where the winner is $1$: 
$$A_1 = \left\{X_1 \ge \max_{j > 1} X_j\right\}.$$
For convenience, we let
$$D_{jk} = \frac{X_j - X_k}{2} ~~~~ \text{ and } ~~~~ M_{jk} = \frac{X_j + X_k}{2}.$$

We start by re-parametrizing to replace $X_1$ and $X_j$ with $D_{1j}$ and $M_{1j}$. The distribution is now an exponential family with sufficient statistics $D_{1j}, M_{1j}, X_{\setminus\left\{1, j\right\}}$ and corresponding natural parameters $\theta_1 - \theta_j, \theta_1 + \theta_j, \theta_{\setminus\left\{1, j\right\}}$. We now consider
\begin{equation}
...
We now proceed to show that $p_{12}$, the $p$-value comparing the winner to the runner-up, is the largest of all $p_{1j}$. Without loss of generality, it is sufficient to show that $p_{12} \ge p_{13}$. We can write this as $g_{X_4, \ldots, X_n}\left(X_1, X_2, X_3\right)$. The first point in \Cref{lma:twoprop} says that the function $g_{X_4, \ldots, X_n}$ is Schur-concave as well. This allows us to clean up our notation and just rename $g_{X_4, \ldots, X_n}$ as $g$. The p-values thus are
\begin{align*}
...
\paragraph{3. $p_{12}$ is an unadjusted pairwise $p$-value}
...
Now suppose the exponential family is defined on the counting measure instead. If ties are broken independently and randomly, the end points on the rays can be considered as ``half an atom'' if the coordinates are integers, and a ``full atom'' otherwise.
...
\end{theorem}
\WFcomment{Are we sure this is a new result?} \KHcomment{Nothing found when I looked up a text for stochastic order, and on Google with ``stochastic order exponential family''}
...
A fundamental difference between the two methods is the definition of the threshold. The threshold from \citet{Gupta:1967wg} depends solely on the setup of the model: $m$ and $n$. Our threshold, however, depends on the data itself. Such difference is amplified when $n$ is large. The threshold from \citeauthor{Gupta:1967wg} increases with $n$ even if the added observations are all $0$. In such case, our threshold does not change at all, suggesting more potential test power.\WFcomment{Actually this isn't really why ours is more powerful.}

\WFcomment{Is there any way we can get an asymptotic expression for $d$ in terms of $n$, as $m\to\infty$, and compare it to our procedure's ``effective $d$''? I suspect there is a $\log n$ factor in there that we don't have.} \KHcomment{We will need to get into details about $r^*$ to explain so, which seems like excessive details. I added some intuition on why our test it better. Not sure if it suffices.}
...
By \Cref{lma:union}, we can construct selective exact one-tailed $p$-values $p_{[1]j}^{(\delta)}$ \WFcomment{What do you think of attaching something like a superscript here to make it clear there are different $p$-values for every $\delta$?} for each of these by conditioning on $A_{[1]}$, $M_{[1]j}$ and $X_{\setminus\left\{[1], j\right\}}$, giving us an exact test for $H_{0[1]}$ by rejecting whenever $\max_{j \ne [1]} p_{[1]j}^{(\delta)} < \alpha$.
...
\end{theorem}
\WFcomment{Probably better to define Procedure 3' outside the theorem?}
...
\WFcomment{You're overloading the variable name $M$ here}

and the each $M_j$ depends on $X$ through the order of $X$. The step-down procedure for rejecting $H_{0j}$ is equivalent to selecting the smallest $j$ for which $F \in M_j\left(X\right)$. \WFcomment{More like the smallest $j$ for which we cannot reject...}
...
Here we are only comparing $X_j$ to $X_{j+1}$ by \Cref{sec:winnerbest}. The upper truncation for $X_j$ can be represented by cropping \Cref{fig:compare_rays} along a vertical line, shown in \Cref{fig:crop_rays}, thus the proof of Part 1 of \Cref{thm:main} on sufficiency\WFcomment{Careful about the word ``sufficiency'', it's a bit of a loaded term in stats} to compare $X_1$ and $X_2$ remains valid with appropriate conditioning. Again we can construct the $p$-values as in \Cref{eqn:p1j} for $k>j$.
...
$$\PP_\theta \left(p_j \le \alpha \mid \theta_{[i]} \text{ is not the } i\text{th largest, some } i\leq j\right) \le \alpha \text{ a.s.}, \quad \forall \; \theta$$
$$\PP_F \left(p_j \le \alpha \middle| M_{j-1}\left(X\right)\right) \le \alpha \text{ a.s.}, ~~~~~~~~ F \in M_{j-1}\left(X\right)$$
by tower property. \WFcomment{Careful on notation again for ``$F\in M_{j-1}$,'' the right hand side is random.}
...
\end{tabular}
\end{center}
\caption{The number of study participants who chose each value as their top desired one. Since only one option can be chosen, these observations are inherently multinomial.}
\label{tbl:fame}
\end{table}

A sample of $20$ students from elementary and middle schools were asked to choose a top ``value'' from 7 different options: achievement, benevolence, fame, community feeling, financial success, self-acceptance, image and other. The results are tabulated in \Cref{tbl:fame}. The study postulates that fame is significantly the top value of choice. It proceeds to perform a binomial test for the null hypothesis that the participants chose among the $7$ options uniformly and concluded with a $p$-value of $0.006$ that fame is indeed the top desired value. The analysis failed to address the ``winner's curse'', as well as the dependence among the observations. 

We have provided a simple, powerful and straightforward test based on comparing the winner to the runner-up. 

comparing the number of ``Fame'' responses to the distribution


 --- perform a unadjusted (two-tailed) binomial test of the winner and the runner-up. In this case, we test if $8$ is a significant observation in $\text{Binomial}\left(8 + 5, 0.5\right)$. The two-tailed $p$-value is $0.580811$, suggesting that the study was inconclusive.
\end{comment}
