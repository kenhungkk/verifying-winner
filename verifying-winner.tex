...
\label{thm:main}
\end{theorem}
\WFcomment{You seem to restate each part of this theorem later in the paper, so that e.g. Theorem 4 is the same as part 1 of Theorem 1.}
...
where $g\left(x\right)$ is a function that counts the number of possible tournament results giving the net win vector $x$. A bijection proof shows that $x$ is indeed Schur-concave. \WFcomment{What is a bijection proof?} Therefore, we can use Procedures 1--3 to compare player qualities.
...
\begin{proof}
\WFcomment{I don't think this is a new result. In particular, it's a special case of the partial conjunction null test, but it should also predate that paper.\\

Careful on terminology: the term ``exact'' usually means ``non-asymptotic'' as opposed to ``exactly uniform.'' Also, I was thrown at first when you said ``since $p_j$ are all valid $p$-values.'' I know what you mean now but at first I thought you were saying all the null hypotheses were true.}

Since $p_j$ are all valid $p$-values,
$$\PP_{H_{0j}}\left(p_j < \alpha\right) \le \alpha.$$
So for the union null hypothesis
...
\WFcomment{What is $\PP_{H_{0j}}\left(p_* < \alpha\right)$? That is not a well-defined probability because it depends on the distribution of the other p-values (I suppose the same could be said for $\PP_{H_{0j}}(p_j < \alpha)$, but at least that won't confuse anyone). Anyway, the second claim in this Lemma isn't true: if all the p-values are uniform, then the max p-value is not uniform.}
\end{proof}
...
\label{thm:seltest}
\end{theorem}
\WFcomment{This is just a restatement of conclusion 1 of Theorem 1, right?}

\begin{proof}
\WFcomment{I've now read this proof carefully and it looks generally correct, but I wonder if we could organize it a little more clearly?}

Without loss of generality, otherwise by relabeling, we consider only the case where the winner is $1$ and the runner-up is $2$, etc., i.e.\ $X_1 \ge \cdots \ge X_n$\WFcomment{and that ties are broken such that... need to figure out how to say this right. Also, we should be a little more careful how we say ``without loss of generality'' --- sounds like you're saying we can relabel indices after observing the data?}. In this case, we will test the null hypothesis $H_{01}: \theta_1 \le \max_{j > 1} \theta_j$, which is the union of the null hypotheses $H_{01j}: \theta_1 \le \theta_j$ for $j > 1$. For each of these we can construct an exact $p$-value $p_{1j}$. Hence by \Cref{lma:union}, a test based on $p_{1*} = \max_j p_{1j}$ would be exact for $H_{01}$\WFcomment{No; see note. However, because $p_{1*}=p_{12}$ almost surely given $X_1>X_2>\cdots$, $p_{1*}$ actually is conditionally uniform.}. \WFcomment{As we will see, } Procedure 1 will perform an unadjusted pairwise test comparing $X_1$ to $X_2$. Hence it is sufficient to show that $p_{12} = p_{1*}$ and that the test based on $p_{12}$ coincides with the unadjusted pairwise test.

Following the framework in \citet{Fithian:2014ws}, we consider the selection event where the winner is $1$: \WFcomment{Did we already condition on $X_1\ge X_2\ge\cdots\ge X_n$? See prev. comment.}
$$A_1 = \left\{X_1 \ge \max_{j > 1} X_j\right\}.$$
For convenience, we let
$$D_{jk} = \frac{X_j - X_k}{2} ~~~~ \text{ and } ~~~~ M_{jk} = \frac{X_j + X_k}{2}.$$

We start by re-parametrizing to replace $X_1$ and $X_j$ with $D_{1j}$ and $M_{1j}$ \WFcomment{Worth pointing out we now have an exponential family in $D_{1j},M_{1j},\cdots$ with nat. param.s ...?} and consider
\begin{equation}
...
\end{align*}
\WFcomment{What happened to the other $\theta_j$ values? I think the first ``$\sim$'' above should be a ``$\propto$'' and you need to say where all the $\theta$'s went.} 
...
Taking expectation on both sides gives \WFcomment{``Marginalizing over $M_{1j},X_{\setminus{1,j}}$,'' maybe?}
$$\PP_{H_{01j}}\left(p_{1j} < \alpha \middle| A_1\right) = \alpha.$$
Therefore these $p_{1j}$ are indeed valid exact $p$-values.

Without loss of generality, it is sufficient to show that $p_{12} \ge p_{13}$. In the computation of either $p$-value, $X_4, \ldots, X_n$ are all conditioned on. The first point in \Cref{lma:twoprop} says that we may assume $n = 3$ without affecting the Schur-concavity of $g$. \WFcomment{Again, exactly what are we doing here? I think we're not really assuming $n=3$; more like conditioning on the other variables to reduce to a case where $n=3$, but we need to be explicit about how this reduction works.} This allows us to clean up our notation and
\begin{align*}
...
Suppose $X_2 < M_{13}$. \WFcomment{Changing variables to $z + D_{12}$ and $z + D_{13}$ respectively}, we can re-parametrize the integrals above to give
\begin{align*}
...
\end{tikzpicture}
\end{center}
\caption{The $p$-value $p_{12}$ can be written in terms of integral $A$ along the segment and $B$ along the ray. The diagram is drawn a level set of $x_1 + x_2 + x_3$ \WFcomment{Problem: the point $(1,0,0)$ is not actually in that level set.}. The green region represents the selection event $A_1$.}
\label{fig:p-value}
\end{figure}
...
\end{tikzpicture}
\end{center}
\caption{The $p$-value $p_{12}$ can be written in terms of integral $A$ along the segment and $B$ along the ray; and $p_{13}$ in terms of $C$ and $D$. $A'$ would refer to the sum of $A$ with the dashed line portion labeled as $\tilde{A}$, formally explained in \Cref{eqn:int_extension}. The majorization relation is indicated by the dotted line. \WFcomment{The point $(X_1,X_2,X_3)$ is cut off a bit in the picture.}}
...
For the alternative case where $X_2 \ge M_{13}$, the segment $C$ will reach the line $x_1 = x_2$ first before it reaches $x_1 = x_3$, ending at $\left(X_2, X_2, X_1 - X_2 + X_3\right)$ instead. But we can still extend $A$ by $\tilde{A}$ to $\left(X_2, X_1, X_3\right)$. The rest of the proof follows. In either cases, $p_{12} \ge p_{13}$, or in generality, $p_{12} \ge p_{1j}$ for $j > 1$. In other words, $p_{12} = p_{1*}$.
...
The proof above holds true if the exponential family is defined on the counting measure. If ties are broken independently and randomly, the end points on the rays can be considered as ``half an atom'' if the coordinates are integers, and a ``full atom'' otherwise. \WFcomment{Should be part of the proof.}
...
\end{theorem}
\WFcomment{Under what conditions? For a multivariate exponential family with a symmetric carrier distribution, right? Also, are we sure this is a new result?}
...
\label{sec:subsetsel}
\WFcomment{Section title can be improved: this section is primarily about a power comparison with the most natural competing procedure.}
...
\WFcomment{Why is $D$ capitalized? It's not a random variable, is it? Also, can you explain how $D$ is selected? e.g., ``Here, we take $d(m,n,\alpha)$ to be the smallest integer such that \eqref{eq:subsetsel} holds for any $\pi$.''}
It also provides the method to choose $D$. The subset selection can be modified. Inherently, the event $\left\{J\left(X\right) = \left\{1\right\}\right\}$ is the same as inferring $\pi_1$ is the largest. We thus implemented their test for this purpose. \WFcomment{The phrasing here is awkward because the relationship between successive sentences is not clear. Instead try something like: ``Subset selection is closely related to testing whether the winner is the best. In particular, we can define a test that declares $j$ the best whenever $\{J(X)=\{j\}\}$. If $J(X)$ satisfies \eqref{eq:subsetsel}, this test is valid at level $\alpha$. We next compare the power of the resulting test against the power of our Procedure 1 in a multinomial example with $pi \propto \left(e^\delta, 1, \ldots, 1\right)$, for several combinations of $m$ and $n$.''}
...
\WFcomment{Is there any way we can get an asymptotic expression for $d$ in terms of $n$, as $m\to\infty$, and compare it to our procedure's ``effective $d$''? I suspect there is a $\log n$ factor in there that we don't have.}

\WFcomment{``To interpret, e.g., the lower left plot, suppose that in a poll of $m=250$ respondents, one candidate enjoys $20\%$ support and the other $n-1=9$ split the remainder ($\smash{\delta = \log\frac{0.2}{0.8 / 9} \approx 0.69}$). Then our procedure has power XXXX to detect the best candidate, while Gupta \& Nagel's procedure has power YYYY.''}

\section{By How Much?}
\label{sec:confbound}
\WFcomment{I like that your section heading recalls the earlier discussion, but this feels a little informal for a section heading.}
...
Our selective test procedure allows us to untangle multiple comparisons based on correlated observations easily, while avoiding the pitfalls of selective inference \WFcomment{This sentence is confusing}. To illustrate, we look at a past psychology study done by \citet{Uhls:2012gf}.
