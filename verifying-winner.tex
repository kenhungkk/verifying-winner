...
\begin{aug}
  ...
  \address{Department of Statistics\\ 301 Evans Hall\\ Berkeley, CA 94720\\
          \printead{e2}}
...
\begin{enumerate}
\item Procedure 1 has exact level $\alpha$ conditional on $H$ being true (conditional on the best population not winning), and marginally has level $\alpha\PP(H \text{ is true}) \leq \alpha \left(1-\frac{1}{n}\right)$.
...
Note that Theorem~\ref{thm:main} implies that we could actually replace $\alpha$ with $\alpha \frac{n}{n-1}$ to obtain a more powerful version of Procedure 1 when $n$ is not too large.
...
\Cref{sec:maj} defines Schur concavity, and gives several examples satisfying this condition. \Cref{sec:winnerbest} justifies Procedure 1 and compares its power to that of \citet{Gupta:1967wg}. \Cref{sec:confbound,sec:stepwise} justify Procedures 2 and 3 respectively, and \Cref{sec:disc} concludes.
...
\begin{theorem}
Procedure 3' is a stepwise procedure that an estimate $\hat{j}_0$ of $j_0$ at the FWER controlled at $\alpha$, where $j_0$ is given by
