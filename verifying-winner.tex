...
\begin{algorithm}[htbp]
	...
	$X_{[0]} \leftarrow \infty$\;
	$j \leftarrow 0$\;
	\Rejected $\leftarrow$ \True\;
	\While{\Rejected}{
		$j \leftarrow j+1$\;
		$D_{[j][j+1]} \leftarrow X_{[j]} - X_{[j+1]}$\;
		Set up the distribution of $D_{[j][j+1]}$, conditioned on
		\begin{itemize}
		\item the variables $X_{[1]}, \ldots, X_{[j-1]}, X_{[j+2]}, \ldots, X_{[n]}$, and
		\item the event $\left\{X_{[j-1]} \ge X_{[j]} \ge \max_{k>j} X_{[k]}\right\}$\; \label{lne:event}
		\end{itemize}
		\Comment{The distribution of $D_{[j][j+1]}$ depends only on $\theta_{[j]} - \theta_{[j+1]}$ now}
		\Test $H_0: \theta_{[j]} - \theta_{[j+1]} \le 0$ against $H_1: \theta_{[j]} - \theta_{[j+1]} > 0$ according to the distribution of $D_{[j][j+1]}$\;
...
Later we will see that Procedure 3 is equivalent to Procedure 3' for the most part, except that Procedure 3 conditions on a larger event $\left\{X_{[j]} \ge \max_{k>j} X_{[k]}\right\}$ in \Cref{lne:event}.
...
\begin{proof}
We will first show that Procedure 3' falls into the sequential goodness-of-fit testing framework proposed by \citet{Fithian:2015uj}. Then we will show that it satisfies the subpath sufficiency principle (SSP) and construct the $p$-values. These constructions suffice to show that FWER is controlled by Theorem 4 in \citet{Fithian:2015uj}.

\paragraph{Application of the sequential goodness-of-fit testing framework}

Upon observing $X_{[1]} \ge \cdots \ge X_{[n]}$, we can set up a sequence of nested model
...
In other words, $\mathcal{M}_j\left(X\right)$ is a family of distributions that does not have all first $j$ ranks correct. As we will see later, each step in Procedure 3' corresponds to testing $\widetilde{H}_{0j}$, stating that without the first $j$ ranks correct, it is hard to explain the observations. Thus, returning $\hat{j}_0 = j$ amounts to rejecting $\widetilde{H}_{01},\ldots,\widetilde{H}_{0j}$, or equivalently determining that the models $\mathcal{M}_1(X), \ldots, \mathcal{M}_j(X)$ do {\em not} fit the data.

\paragraph{Demonstration of the SSP} As in the proof of Parts 1 and 2 of \Cref{thm:main} before, we drop the square brackets subscripts for convenience here, while maintaining that the sample ranks are random. Under the model $\mathcal{M}_j\left(X\right)$, the minimal sufficient statistic is simply $X$. The sufficient filtration \citep{Fithian:2015uj} is thus
$$\mathscr{F}_j = \sigma\left(\mathcal{M}_0\left(X\right), \ldots, \mathcal{M}_j\left(X\right), X\right) = \sigma\left(X\right),$$
where $\mathcal{M}_k\left(X\right)$ above refers to the choice of the model as a random variable, and thus measurable in $\sigma\left(X\right)$. This is the same as $\sigma\left(\mathcal{M}_j\left(X\right), X\right) = \sigma\left(X\right)$ for all $j$. Thus the random sequence of model satisfies SSP.

Now if we construct valid selective $p$-values, the $p$-values sequence will be independent on nulls, as defined and shown in Theorem 4 of \citet{Fithian:2015uj} and FWER will be controlled.

\KHcomment{It occured to me that the minimal sufficient statistic should be the entire $X$, as the sequence of model does not assert $\theta_j = \theta_{j+1}$.}

\paragraph{Construction of the selective $p$-values}

As defined in \citet{Fithian:2015uj}, a selective $p$-value $p_j$, for testing $H_{0j}: \theta \in \mathcal{M}_j\left(X\right)$ is valid if and only if
$$\PP_\theta\left(p_j \le \alpha \;\middle|\; \mathcal{M}_j\right) \le \alpha \text{ a.s.\ on } \left\{\theta \in \mathcal{M}_j\left(X\right)\right\}.$$

\KHcomment{Testing $H_{0j}$ is not the same $\widetilde{H}_{0j}$, need fixes.}

By smoothing, we can construct selective $p$-values by conditioning on $\mathcal{M}_j\left(X\right)$, as well as other variables corresponding to the nuisance parameters. Specifically, we are conditioning on the choice of the model $\mathcal{M}_j\left(X\right)$ and the variables $X_1, \ldots, X_{j-1}, X_{j+1}, \ldots, X_n$ and $M_{j\left(j+1\right)}$ as defined in earlier in the proof of Part 1 of \Cref{thm:main}. We can take $p_j$ to be the survival function of the conditional law
\begin{align}
...
\end{tikzpicture}
\end{center}
\caption{The two $p$-values constructed corresponds to taking integrals of $g$ along these segments, that lie on a level set of $x_j + x_{j+1} + x_k$. The dashed line corresponds to extension in \eqref{eqn:int_extension}. The dotted line on the far right is the truncation that enforces $X_j < X_{j-1}$.}
\label{fig:crop_rays}
\end{figure}

Here we are only comparing $X_j$ to $X_{j+1}$ by \Cref{sec:winnerbest}. The upper truncation for $X_j$ can be represented by cropping \Cref{fig:compare_rays} along a vertical line, shown in \Cref{fig:crop_rays}. Thus the proof of Part 1 of \Cref{thm:main} that it suffices to compare $X_1$ and $X_2$ remains valid. Again we can construct the $p$-values as in \Cref{eqn:p1j} for $k>j$.
...
\paragraph{FWER is controlled}

\WFcomment{You've already said directly that we are conditioning on $X_{[1]}$ through $X_{[j-1]}$. More generally, if we can sidestep the more generic language of that paper and substitute more specific language for our specific problem, I think that will make this a lot more readable.}
\end{proof}
