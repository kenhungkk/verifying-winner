...
Without loss of generality, it is sufficient to show that $p_{12} \ge p_{13}$. In the computation of either $p$-value, $X_4, \ldots, X_n$ are all conditioned on. Fixing these variables that are conditioned on, $g$ has only thee variables: $X_1$, $X_2$ and $X_3$. We can write this as $g_{X_4, \ldots, X_n}\left(X_1, X_2, X_3\right)$. The first point in \Cref{lma:twoprop} says that the function $g_{X_4, \ldots, X_n}$ is Schur-concave as well. This allows us to clean up our notation and just rename $g_{X_4, \ldots, X_n}$ as $g$. The p-values thus are
\begin{align*}
...
Here we are only comparing $X_j$ to $X_{j+1}$ by \Cref{sec:winnerbest}. The upper truncation for $X_j$ can be represented by cropping \Cref{fig:compare_rays} along a vertical line, shown in \Cref{fig:crop_rays}, thus the proof of Part 1 of \Cref{thm:main} on sufficiency to compare $X_1$ and $X_2$ remains valid with appropriate conditioning. Again we can construct the $p$-values as in \Cref{eqn:p1j} for $k>j$.
...
Our selective test procedure allows us to untangle multiple comparisons based on correlated observations easily, while avoiding the pitfalls of selection. To illustrate, we look at a past psychology study done by \citet{Uhls:2012gf}.
