...
Once we see the results of the poll, several salient questions will likely occur to us: Is Trump really winning, and if so by how much? And analogously, is Cruz really in second, is Rubio really in third, and so on?

Note that there is implicitly a problem of multiple comparisons here, since we would be asking a different set of questions if Cruz had led the poll instead. Indeed, the selection issue appears especially pernicious due to the so-called ``winner's curse:'' given that Trump leads the poll, it more likely than not overestimates his support.
...
\begin{enumerate}
\item {\em Is Trump really winning?} If Trump and Cruz were in fact tied, then Trump's share of their combined 490 votes should be distributed as $\text{Binom}(490, 0.5)$. Because the (two-tailed) $p$-value for this test is $p=0.006$, we can reject the null and declare that, yes, Trump is really winning.
\item {\em By how much?} Using an exact $95\%$ interval for the same binomial model, we conclude Trump has at least $7.5\%$ more support than Cruz (and by extension everyone else).
...
Surprisingly, the three procedures described above are all entirely valid despite the implicit multiple-comparisons issue. The remainder of this article is dedicated to justifying them.


\begin{enumerate}

\item This has a hidden upward selection bias, or ``winner's curse'', as we are only testing this hypothesis as Trump is leading in the polls. In other words, the hypothesis itself is random. Alternatively, we can note that the marginal type I error rate is indeed not so straightforward. Specifically, the null hypothesis is ``$H_0 \left(X\right): \text{Leader in polls has most support}$'', which is random and dependent on poll data $X$. Then for any distribution $F$ the marginal type I error rate is
...
when $X_j$ is larger than all other $X_k$. In case of ties, we break them independently and randomly. The distribution, written in an exponential family format, is
...
where (i) follows from $H_{01} = \bigcup_{j>1} H_{01j}$; (ii) is true as $p_{12} \ge p_{1j}$ as long as $X_1 \ge X_2 \ge \max_{j>2} X_j$, which is guaranteed here as we conditioned on $A_1$; (iii) follows from the construction of $p_{1j}$. \KHcomment{I think the probability in the middle (between (i) and (ii)) is well-defined, as $p_{12}$ is just a random variable dependent on $X$.} Thus it is sufficient to construct a one-sided test between $X_1$ and $X_2$ and consider only $p_{12}$.
...
\end{equation}
defined on the Lebesgue or counting measure. It is reasonable to assume that the family is ``symmetric'', i.e.\ if $\theta_j = \theta_k$, then the probability distribution remains the same when $X_j$ and $X_k$ are swapped. This corresponds to requiring $g\left(X\right)$ to be symmetric. We would also like to introduce a fairly mild additional condition that $g$ is {\em Schur-concave}. But first we recall the notion of {\em majorization}, defined on both $\mathbb{R}^n$ and $\mathbb{Z}^n$.
\begin{definition}
...
Now suppose the carrier distribution $g$ in \Cref{eqn:exp_family} is indeed Schur-concave. We hereon call the class of distribution satisfying this {\em Schur-concave exponential family}. Without loss of generality, otherwise by relabeling, we consider only the event $X_1 \ge X_2 \ge \max_{k > 2} X_k$. We will be testing the null hypothesis $H_{01}: \theta_1 \le \max_{j > 1} \theta_j$, which is the union of the null hypotheses $H_{01j}: \theta_1 \le \theta_j$ for $j > 1$.
...
\begin{tikzpicture}
	...
	\draw[thick, dashed] (A) -- (D) node[midway, below left] {$\hat{A}$};
